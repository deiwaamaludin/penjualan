...
class adminweb extends CI_Controller {
   ...
   //Akhir Sosial Media
	...
	public function merk() {
		...
		$data['data_merk']= $this->admin_model->GetMerk();
		$this->template->load('template','adminweb/merk/index',$data);
	}

	public function merk_simpan() {
		$nama_merk = $this->input->post("nama_merk");
		$cek = $this->admin_model->MerkSama($nama_merk);

		if ($cek->num_rows()>0) {
			$success = "1";
		}
		else {
			$this->session->set_flashdata('berhasil','	Merk Berhasil Disimpan');
			$this->admin_model->MerkSimpan($nama_merk);
			$success="0";
		}
	...
	public function merk_edit() {
		$id_merk = $this->uri->segment(3);
		$query = $this->admin_model->GetEditMerk($id_merk);
		foreach ($query->result_array() as $tampil) {
			$data['id_merk'] = $tampil['id_merk'];
			$data['nama_merk'] = $tampil['nama_merk'];
		}

		$this->template->load('template','adminweb/merk/edit',$data);
	}

	public function merk_delete() {
		$id_merk = $this->uri->segment(3);
		$this->admin_model->DeleteMerk($id_merk);

		$this->session->set_flashdata('message','Merk Berhasil Dihapus');
		redirect("adminweb/merk");
	}

	public function merk_update() {
		...
		if ($cek->num_rows()>0) {
			...
			$this->session->set_flashdata('berhasil','Merk Berhasil Disimpan');
			$this->admin_model->MerkUpdate($id_merk,$nama_merk);
			$success="0";
		}
	...
	public function produk_simpan() {
		...
		$this->form_validation->set_rules('merk_id','Merk','required');
			...
			if(empty($_FILES['userfile']['name']))
				{
						...
						$in_data['merk_id'] = $this->input->post('merk_id');
